...
export const BaseItemSchema = z.object({
  ...
  rarity: z.enum(CapitalizedRarity),
  // .transform((rarity) => rarity.toLowerCase() as RarityType),
  icon: z.string(),
  item_type: z.string(),
  value: z.number(),
  ...
  ammo_type: z
    ...
    .nullish()
    .transform((val) => val ?? undefined),
  stat_block: StatSchema,
  components: z.array(ChildComponentSchema),
  used_in: z.array(ChildItemSchema),
  recycle_components: z.array(ChildComponentSchema),
  recycle_from: z.array(ChildItemSchema),
  created_at: z.iso.datetime({ offset: true }).pipe(z.coerce.date()),
  updated_at: z.iso.datetime({ offset: true }).pipe(z.coerce.date()),
});

export type ApiItem = z.infer<typeof BaseItemSchema>;
